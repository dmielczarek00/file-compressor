...
class CompressionWorker:
    def __init__(self, redis_url: str, db_url: str,
        ...
        self._job_cancel_event = asyncio.Event()
                ...
                if job_id:
                    ...
                    if job_details:
                        with self.job_processing_time.labels(
                            ...
                            await self.process_job_with_heartbeat(job_details)
                    else:
    ...
    async def process_job_with_heartbeat(self, job_data: Dict[str, Any]):
        """Process a job while sending periodic heartbeats."""
        job_id = str(job_data.get('uuid'))
        self._current_job_id = job_id
        self._job_cancel_event.clear()
        self._heartbeat_task = asyncio.create_task(self._send_heartbeat(job_id))
        try:
    ...
    async def _send_heartbeat(self, job_id: str):
        """Send heartbeat to DB every heartbeat_interval seconds while job is running."""
        while not self._job_cancel_event.is_set():
            try:
                await self.db.update_job_heartbeat(job_id)
                logger.debug(f"Heartbeat updated for job {job_id}")
            except Exception as e:
                logger.warning(f"Failed to update heartbeat for job {job_id}: {e}")
            await asyncio.wait([self._job_cancel_event.wait()], timeout=self.heartbeat_interval)
